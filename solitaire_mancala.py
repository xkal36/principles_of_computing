...
class SolitaireMancala:
    ...
    def is_game_won(self):
        ...
        """
        return all(map(lambda x: x == 0, self._board[1:]))
    ...
    def plan_moves(self):
        ...
        seq = []
        while not done:
            if new_board.is_game_won() or new_board.choose_move() == 0:
                done = True
                return seq
            else:
                seq.append(new_board.choose_move())
                new_board.apply_move(new_board.choose_move())
...
# Run the GUI:
mancala_gui.run_gui(SolitaireMancala())
